...
```
nvim/
├── init.lua              # Entry: require("jwe")
├── lua/
│   ├── jwe/              # Personal config module
...
| Add keymap | `lua/jwe/keymaps.lua` |
| Change option | `lua/jwe/options.lua` |
...
typescript-tools (TS/JS), lua_ls (+ lazydev), rust_analyzer, tailwindcss, svelte, biome, eslint (autostart=false)
